"""
Fix duplicate baseline entries in MongoDB - Production Safe

Run this on Railway to fix the duplicate baseline issue.
"""
import asyncio
...
async def fix_duplicate_baselines():
    """Remove duplicate baseline entries, keeping only the most recent one"""
    ...
    for dup in duplicates:
        ...
        print(f"   ✅ Keeping baseline: {keep_doc['_id']}")
        if 'created_at' in keep_doc:
            print(f"      Created: {keep_doc['created_at']}")

        for doc in delete_docs:
            print(f"   🗑️  Deleting baseline: {doc['_id']}")
    ...
    # Now try to recreate the unique index
    print("\n🔧 Fixing index on kvk_season_id...")
    try:
